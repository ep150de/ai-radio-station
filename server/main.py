...
async def export_m3u():
    """
    Export the current non-repeating playlist order as a standard .m3u playlist.
    Useful for playing the exact same sequence in other players (VLC, etc.).
    """
    st = playlist_manager.state
    ...
    lines = ["#EXTM3U", "#EXTENC:UTF-8", f"# Playlist generated by AI Radio Station - {STATION_NAME}"]
    lines.append(f"# Total tracks: {len(st.order)} | Cycle: {st.cycle}")
    ...
    for tid in st.order:
        ...
        # Use relative path from the music root. Users can adjust this when loading the playlist.
        # This is the most useful format for personal use.
        lines.append(track.path.replace("\\", "/"))

    content = "\n".join(lines) + "\n"

    filename = f"beacon-fm-playlist-{st.cycle}.m3u"
...
# --- Phase 3: Favorites API ---------------------------------------------

@app.get("/api/favorites")
async def get_favorites():
    fav_ids = playlist_manager.get_favorites()
    lib = _load_library()
    id_to_track = {t.id: t for t in lib}
    favorites = [id_to_track[tid].model_dump() for tid in fav_ids if tid in id_to_track]
    return {"favorites": favorites, "count": len(favorites)}


@app.post("/api/favorites/{track_id}/toggle")
